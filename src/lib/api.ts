...
export const getLeaves = async (schoolId?: string): Promise<Leave[]> => {
  ...
  return (data || []).map(dbLeave => ({
    ...
    date: dbLeave.date,
    type: (dbLeave.type === 'FULL' ? 'FULL_DAY' : 'PARTIAL_DAY') as LeaveType,
...
export const addLeave = async (leave: Omit<Leave, 'id'>): Promise<void> => {
  const { error } = await supabase
    .from('leaves')
    .insert({
      ...
      date: leave.date,
      type: leave.type === 'FULL_DAY' ? 'FULL' : 'PARTIAL',
...
export const addRequest = async (request: Omit<Request, 'id'>): Promise<void> => {
  const { error } = await supabase
    .from('requests')
    .insert({
...
export const addHistory = async (history: Omit<SubstitutionHistory, 'id'>): Promise<void> => {
  const { error } = await supabase
